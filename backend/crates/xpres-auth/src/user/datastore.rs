...
impl<Conn: 'static + DBConnection> From<Conn> for Datastore {
    fn from(db: Conn) -> Datastore {
        Datastore {
            conn: Box::from(db),
...
impl<T0: 'static + DBConnection, T1: 'static + SessionManager> From<(T0, T1)> for Datastore {
    fn from((db, ss): (T0, T1)) -> Datastore {
        Datastore {
            conn: Box::from(db),
            sess: Box::new(ss),
        }
